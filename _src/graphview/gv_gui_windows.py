...
class NodeList(Window):
    ...
    def react(self,evt):
        ...
        if 'select' in evt.type:
            self.field.unselect()
        if 'add' in evt.type or  'change' in evt.type or 'rem_info' in evt.type:
            self.renew()
...
class EditorMenu(DragWindow):
    ...
    def clear(self):
        ...
        self.chgevt={}
        self.delfield={}
        self.delevt={} #Events for removing an info (only in transparent data)
    ...
    def make(self,**kwargs):
        ...
        for a in self.attrs :
            ...
            if (not self.itemwrite and self.data.transparent and self.ref
...
#            field.unbind_command()
    ...
    def del_info(self,attr):
        '''When the infosource is transparent, delete local info.'''
        if self.delfield[attr].is_disabled:
            return False
        evt=self.delevt[attr]
        user.evt.do(evt,self,1,ephemeral=self.ephemeral)
