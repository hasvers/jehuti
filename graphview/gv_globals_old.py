...
def confload(fname): #Very nasty way of loading the .ini
    ...
    entry=eval(fname)
    try :
        f = fopen(database['basepath']+fname+'.ini','rU')
    except :
        f = fopen(database['srcpath']+fname+'.ini','rU')
    text='''{}'''.format(f.read())
...
database['srcpath']=resource_path(database['srcpath'])


try:
    basefont = pg.font.Font(os.path.normpath( database['image_path']+"fonts/"+database['font']), database['font_default_size'])
    emotefont = pg.font.Font(os.path.normpath( database['image_path']+"fonts/"+database['font']), database['font_emote_size'])
except IOError as e:
...
def shallow_nested(item,make_new=0):
    ...
    if hasattr(item,'nested_copy') and not item.nested_copy:
        #Tricky: I want databits to be copiable (e.g. script effect)
        # but not dataitems that serve as reference (e.g. node),
        # neither fields contained in those dataitems
        make_new=0
    if hasattr(item,'update'):
        new= item.__class__()
        new.update({ shallow_nested(i,make_new):shallow_nested(j,make_new) for i,j in item.iteritems()})
        return new
    if hasattr(item,'__iter__'):
        ...
        new= item.__class__()
        for i in item :
            new+=  item.__class__( ((shallow_nested(i,make_new)), ))
        return new
    if make_new and hasattr(item,'__dict__') :
        new= item.__class__()
        for i,j in item.__dict__.iteritems():
            new.__dict__[i]=shallow_nested(j,make_new)
        return new
    return item