...
class StartMenuUI(MenuUI):
    ...
    def matchinit(*args,**kwargs):
        match = MatchEditor()
        if 0 :
            done=False
            fin=open('editor.log','r')
            for i in fin :
                j = i.split(':')
                if j[0]=='current_match':
                    try:
                        open(j[1],'r')
                        done=True
                        match.set_from_file(j[1])
                    except:
                        break
                    #except:
                    #    match.make()
            ...
            pass
        if not 'bg' in kwargs:
            kwargs['bg']=database['backgrounds']+database['default_bg']
        match.setting.set_background(kwargs['bg'])
        return match.data
